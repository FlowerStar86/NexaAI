...
import { recommendWithAi, fetchCareerFromAgents, type AiRecommendResult } from "@/lib/ai-recommend.functions";
...
function Results() {
  ...
  const run = async (p: Profile) => {
    ...
    try {
      let r: AiRecommendResult;
      try {
        r = await runRecommend({ data: payload });
      } catch {
        r = await fetchCareerFromAgents(payload);
      }
      setResult(r);
      saveAiResultLocal(r);
    } catch (e) {
        ...
        {result.agent_outputs && (
          <section className="space-y-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-semibold">Python Agent Analysis Reports</h2>
                <p className="text-xs text-muted-foreground">Direct intelligence generated by your specialized Python agents</p>
              </div>
            </div>

            <div className="grid gap-6 md:grid-cols-3">
              {result.agent_outputs.skill_gap && (
                <div className="rounded-2xl border border-border bg-surface p-6 shadow-card">
                  <h3 className="font-display text-lg text-brand">Skill Gap Agent Report</h3>
                  <pre className="mt-3 text-xs whitespace-pre-wrap text-muted-foreground font-mono leading-relaxed max-h-60 overflow-y-auto">
                    {result.agent_outputs.skill_gap}
                  </pre>
                </div>
              )}
              {result.agent_outputs.cv_analysis && (
                <div className="rounded-2xl border border-border bg-surface p-6 shadow-card">
                  <h3 className="font-display text-lg text-brand">CV Agent Report</h3>
                  <pre className="mt-3 text-xs whitespace-pre-wrap text-muted-foreground font-mono leading-relaxed max-h-60 overflow-y-auto">
                    {result.agent_outputs.cv_analysis}
                  </pre>
                </div>
              )}
              {result.agent_outputs.job_readiness && (
                <div className="rounded-2xl border border-border bg-surface p-6 shadow-card">
                  <h3 className="font-display text-lg text-brand">Job Readiness Agent Report</h3>
                  <pre className="mt-3 text-xs whitespace-pre-wrap text-muted-foreground font-mono leading-relaxed max-h-60 overflow-y-auto">
                    {result.agent_outputs.job_readiness}
                  </pre>
                </div>
